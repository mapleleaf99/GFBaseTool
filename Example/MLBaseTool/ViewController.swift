...
class ViewController: UIViewController {
    ...
    private let demos: [DemoItem] = [
        ...
        DemoItem(title: "Keychain 存储", subtitle: "Token 等敏感信息安全存储") {
            KeychainDemoViewController()
        },
        DemoItem(title: "文件与缓存", subtitle: "缓存大小、清理缓存") {
            FileDemoViewController()
        },
        DemoItem(title: "权限管理", subtitle: "相机、相册、定位、麦克风") {
            PermissionDemoViewController()
        },
        DemoItem(title: "键盘处理", subtitle: "点击收起、ScrollView 避让") {
            KeyboardDemoViewController()
        },
        DemoItem(title: "空状态视图", subtitle: "列表无数据、失败重试") {
            EmptyViewDemoViewController()
        },
        DemoItem(title: "Alert 弹窗", subtitle: "MLAlertObject 队列与优先级") {
            AlertDemoViewController()
        },
        DemoItem(title: "防抖与节流", subtitle: "Debouncer / Throttler") {
            DebouncerDemoViewController()
        },
        DemoItem(title: "自定义控件", subtitle: "MLButton、MLTextField、MLDottedLine") {
