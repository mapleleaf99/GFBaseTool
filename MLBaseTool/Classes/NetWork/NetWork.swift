...
public class NetWork: NSObject {
    ...
    /// 全局网络配置
    public var configuration: MLNetworkConfiguration {
        get { MLNetworkConfiguration.shared }
        set { MLNetworkConfiguration.shared = newValue }
    }
                ...
                case .success(let value):
                    if value.code == config.successCode {
                        if let data = value.data {
                            ...
                            let error = MLNetworkError.emptyData
                            Toast.show(error.errorDescription)
                            failure?(error)
                        }
                    } else {
                        let error = MLNetworkError.business(code: value.code, message: value.msg)
