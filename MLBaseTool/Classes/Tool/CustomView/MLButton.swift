...
/// 图文排列方式
public enum MLButtonLayoutType: Int {
    case none = 0       /// 默认：图左文右
...
/// 按下时的动画效果
public enum MLButtonAnimationType: Int {
    case none = 0
    case textScale      /// 文字放大
...
/// 增强型 UIButton
public class MLButton: UIButton {

    public typealias MLButtonBlock = (_ button: MLButton) -> Void
    ...
    /// 图文布局类型
    public var type: MLButtonLayoutType = .none
    /// 按下动画类型
    public var animationType: MLButtonAnimationType = .none

    /// 设置点击回调
    public func buttonClick(_ block: @escaping MLButtonBlock) {
        clickBlock = block
        listenTouchUpInside()
    }

    /// 设置长按回调（触发时震动）
    public func onLongPress(_ block: @escaping MLButtonBlock) {
    ...
    public func longPressBlock(_ block: @escaping MLButtonBlock) {
        onLongPress(block)
    }
    ...
    public convenience init(_ type: MLButtonLayoutType = .none) {
        self.init(type: type, animationType: .none)
    }

    public convenience init(_ animationType: MLButtonAnimationType = .none) {
        self.init(type: .none, animationType: animationType)
    }

    public convenience init(type: MLButtonLayoutType = .none, animationType: MLButtonAnimationType = .none) {
    ...
    private var clickBlock: MLButtonBlock?
    private var longPressHandler: MLButtonBlock?
...
@objc extension MLButton {

    @objc private func highlight(_ button: MLButton) {
    ...
    @objc private func touchUpInsideEvent(_ button: MLButton) {
        responseCount = 0
        callback()
    }
}
